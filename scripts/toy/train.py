...
from fdbm.data.datasets import build_data_loader
from fdbm.models.base import AlignedSB, FDBMToy
from fdbm.training.epoch_fns import train_epoch, test_epoch, inference_epoch_toy
from fdbm.training.updaters import get_optimizer, get_scheduler, get_ema
from fdbm.training.losses import loss_fn_from_args
from fdbm.utils.helper import count_parameters
...
def train(args, train_loader, val_loader, model, optimizer, scheduler, ema_weights=None, log_dir=None):
    ...
    g = get_diffusivity_schedule(g_max=args.max_diffusivity, H=args.H, K=args.K, norm=args.norm).g
    ...
    for epoch in range(args.n_epochs):
        ...
        val_losses = test_epoch(model=model, loader=val_loader, loss_fn=loss_fn, args=args)
        print_msg = f"Epoch {epoch+1}: "
        ...
        if args.inference_every > 0 and (epoch + 1) % args.inference_every == 0:
            inference_metrics = inference_epoch_toy(model=model, g=g, dataset=val_loader.dataset.data, 
...
def main(cmd_args=None):
    ...
    print(f"Train loader mean inspection:", train_loader.dataset.data["initial"].mean(axis=0))
    print(f"Val loader mean inspection:", val_loader.dataset.data["initial"].mean(axis=0))
    print(flush=True)
