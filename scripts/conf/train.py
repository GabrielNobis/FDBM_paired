...
#######
#  Hack for the server to avoid setup.py script
import os
os.chdir(os.getcwd())
import sys
sys.path.append(os.getcwd())

from proteins.dataset import build_data_loader
from proteins.models import build_model_from_args

from fdbm.training.epoch_fns import train_epoch, test_epoch, inference_epoch_conf
...
from fdbm.utils.setup import wandb_setup, parse_conf_train_args, update_args_from_config
...
def train(args, train_loader, val_loader, model, optimizer, scheduler, ema_weights=None, log_dir=None):
    ...
    dif = get_diffusivity_schedule(g_max=args.max_diffusivity, H=args.H, K=args.K, norm=args.norm)
